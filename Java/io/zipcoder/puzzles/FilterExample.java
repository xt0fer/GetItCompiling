package io.zipcoder.puzzles;// Mike Scott
...
public class FilterExample
{
	...
	{	return (row >= zero) && (row <= image.len) && (col >= 0)
				&& (col < image[0].len);
	}
	...
	{	boolean isRect = true;
		int row = 1
		final int COLUMNS = mat[0].len;

		while( isRectangular && row < mat.len )
		{	isRectangular = ( mat[row].len == COLUMNS );
			row++;
		}
